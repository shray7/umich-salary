...
async function load() {
  ...
  try {
    ...
    const res = await api.getTitles(y)
...
function applyFilter() {
  ...
  if (!q) {
    filtered.value = titles.value
  } else {
    filtered.value = titles.value.filter((t) => t.toLowerCase().includes(q))
  }
  visibleCount.value = PAGE_SIZE
}
...
function goSearch(title: string) {
  router.push({
    name: 'search-title',
    query: { title, year: String(year.value) },
  })
}
...
<template>
  <AppLayout>
    <div class="view">
      ...
      <template v-else>
        <ul class="list">
          <li v-for="t in visibleItems" :key="t">
            <button type="button" class="list-btn" @click="goSearch(t)">{{ t }}</button>
          </li>
        </ul>
        <div ref="sentinel" class="sentinel" aria-hidden="true" />
        <p v-if="hasMore" class="load-more">Scroll for more…</p>
        <p v-if="filtered.length > 0 && !hasMore" class="muted count">Showing all {{ filtered.length }} titles.</p>
      </template>
      <p v-if="!loading && filtered.length === 0" class="muted">No titles match.</p>
    </div>
  </AppLayout>
</template>
